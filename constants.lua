...
function insertcomment(orig, force)
    ...
    local x = "\226\128\174"
    if options.Racism and math.random(3) > 1 then
    ...
    print("appended shitty comment:" .. x)
...
local function extract(code, ast)
    ...
    if options.ConstantChance and options.RacismConstPerc > 0 then

        for k = 1, 25 do
            local m = me[math.random(#me)]
            if math.random(options.RacismConstPerc) == 1 and m then
                addConstant(m, index)
            end
        end
    end
