...
local Generate = {Dead = false, Types = {"Function",
    "Boolean", "Loop"}, Functions = {}, LuaFunctions = {"error", "rawget", "rawset", "require", "rawequal", "type", "tonumber", "tostring", "table.remove", "table.insert", "table.sort", "table.concat", "ipairs", "pcall", "print", "assert", "setfenv", "setmetatable", "string.rep", "string.reverse", "string.upper", "string.sub", "string.dump", "string.find", "string.format", "string.gsub", "string.gmatch", "string.len", "string.lower", "string.char", "string.byte", "string.match", "debug.traceback", "getfenv", "getmetatable", "load", "loadstring", "pcall", "coroutine.wrap", "coroutine.resume", "coroutine.running", "coroutine.yield", "coroutine.status", "coroutine.create", "math.exp", "math.rad", "math.random", "math.randomseed", "math.tan", "math.tanh", "math.pow", "math.abs", "math.acos", "math.asin", "math.atan", "math.atan2", "math.sin", "math.sinh", "math.sqrt", "math.deg", "math.fmod", "math.floor", "math.frexp", "math.log", "math.ldexp", "math.cos", "math.ceil", "math.cosh", "math.max", "math.min", "math.modf"}, Memes = {"evan", "Was", "Fucking", "Here", "Godlyness resides in the great evan", "evan was here", "dope", "evan is gooder than coke", "gooder means more than better", "bruh", "grammer iz overrated", "oops", "loser", "did u no tht every obf is crackable?", "lets try loadstring = print", "blinding light of destiny",
        ...
        "cars allow for transport", "did you know that each car on a transport train carry the back of a semi truck? theyre detachable for a reason.", "if you think i just deveolp lego hacks, youre mistaken. evan is known in multiple communities, hacking or forum related, hes there.", "porn is 75% acting", "empty",
        ...
        "until i stop, im unstopable.", "do you like hat dick? yes king HOW GOOD IS THAT DICK OOOH AMAZING HOW GOOD IS THAT DICK BITCH", "this is a judge free zone except when we are judging you.", "do not fear the light", "did you know that humans are not the only spirits to roam our world? there have been dog spirits reported. the most famouse dog spirit can be found at the whailys house",
        "remember when person299 admin script was johnporked by kohls admin script?", "youre officially breathing properly", "your heart is officially beating properly", "chrome incognito might not hide your activity to people standing behind you",
        ...
        "this damn moth is on my screen", "What", "my dumbass dog tried to pick up a wasp... Didnt end well", "Heres how to beat evan: nil", "when will i move on from roblox?", "Im litteraly 2.5 times the average age of a robloxian",
        "For a script, we could try to yes a yes. But the yes won't be yesed until the other yes turns into a proper yes. Though the yes must be configurable.... but should probably be defaulted at 5 yeses. For example; if you had a yes, then your yes should yes itself. But if your yes is not yessing, then it should yes the the yeses yes until it turns into a yes. Well that may not work. I mean if the yes isn't properly yessed into a yes, then it may yes it's other yessed yes, and that could be a really bad yes. But only evan would know if you could yes a yes. Sound like a good yes?",
        ...
        "the drive to magic mountain is long. but their rides makes it worthy of a trip", "fuck you",
        ...
        "my favorite word is yes", "how to hack my toaster?", "taping ram sticks to the side of your router makes it run faster!!", "i taped an i7 processor to my desktop and it ran faster!", "slow pc? download more ram!", "ispep", "there was not one obfuscator that didnt claim to be the best. until evan came along.",
        "this is not a meme", "no u", "nabster got evaned for copyright", "office: no printing large jobs, Co-workers:printing large images of steeve jobs", "my grandmother used to eat half of her meal and complain about the rest until the resturant supervisor made it free.", "how 2 git free pizza: call up pizza place and say your order was fucked up.", "clutter fucking constants",
...
function Generate.Upvalue(branch, rec, nils)
    ...
    if bran > options.MaxBranch and rec then
        return ({Generate.Bool(), '"' .. Generate.Name(math.random(4, 20), branch == 0) .. '"', Generate.Number(), Generate.Table(bran + 1, true), Generate.LuaFunctions[math.random(#Generate.LuaFunctions)], "not nil", '"evan Was Here"'})[math.random(9)] or '"evan Was Here"'
